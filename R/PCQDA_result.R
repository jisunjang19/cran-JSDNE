...
PCQDA_result<-function(x,y){
  row_PCQDA<-data_PCQDA[1:76,]
  set.seed(1)
  model_PCQDA<-subset(row_PCQDA,select=-c(Cluster))
  PCQDA_df<-scale(model_PCQDA)
  PCQDA_df.pca <- prcomp(PCQDA_df)
  PCQDA_df2<-data.frame(Cluster=row_PCQDA$Cluster, PCQDA_df.pca$x)
  ...
  est_PCQDA<-data.frame(Area.DNEunder0.0001, Area.DNEover0.6, MeanDNE.Apex, TotalDNE.TotalPolygonFaces)
  PCQDA_Bind<-rbind(model_PCQDA,est_PCQDA)
