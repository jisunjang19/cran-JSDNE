...
PCLR_result<-function(x,y){
  row_PCLR<-data_PCLR[1:36,]
  set.seed(3)
  model_PCLR<-subset(row_PCLR,select=-c(Cluster))
  PCLR_df<-scale(model_PCLR)
  PCLR_df.pca <- prcomp(PCLR_df)
  PCLR_df2<-data.frame(Cluster=row_PCLR$Cluster, PCLR_df.pca$x)
  ...
  PCLR_Bind<-rbind(model_PCLR,est_PCLR)
