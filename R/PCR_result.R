...
#' It provides the estimated age and doubled standard errors (2SE).
#'
...
PCR_result<-function(x,y){
  row_PCR<-data_PCR[1:76,]
  set.seed(2)
  model_PCR<-subset(row_PCR,select=-c(Age))
  PCR_df<-scale(model_PCR)
  PCR_df.pca <- prcomp(PCR_df)
  PCR_df2<-data.frame(Cluster=row_PCR$Age, PCR_df.pca$x)
  ...
  Area.DNEunder0.0001<-nrow(df_fine)/nrow(df_whole_DNE)
  Area.DNEover0.6<- nrow(df_macro)/nrow(df_whole_DNE)
  MedianDNE.Whole<-median(df_whole_DNE$whole_DNE)
  est_PCR<-data.frame(MedianDNE.Apex, IQRDNE.Apex, MeanDNE.Convex, MeanDNE.Concave, Area.DNEunder0.0001, Area.DNEover0.6, MedianDNE.Whole)
  PCR_Bind<-rbind(model_PCR,est_PCR)
  ...
  PCR_est <- PCR_Bind_est[-76:-1,]
  PCR_pred<-predict(PCR, PCR_est)
  PCR_result<-data.frame(PCR_df_est)
